class Device < ActiveRecord::Base
  ...
  scope :loans_count, -> { select('devices.*, COUNT(loans.id) AS loan_count').joins('LEFT JOIN loans ON devices.id = loans.device_id').group(:id).order('loan_count DESC') }
  ...
  def self.to_csv(options = {})
    CSV.generate(options) do |csv|
      csv << ['#', 'Device name', 'Loans count', 'Top loaning user']
      all.each.with_index do |device, index|
        csv << [index + 1, device.name, device.loans.count, device.get_top_loan_user]
      end
    end
  end
end
